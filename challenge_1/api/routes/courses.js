  ...
  asyncHandler(async (req, res) => {
    ...
    if (course) {
      const sanitizedCourse = {
        ...
        estimatedTime: sanitizeHtml(course.estimatedTime, {
          allowedTags: [],
          allowedAttributes: {},
        }),
        materialsNeeded: sanitizeHtml(course.materialsNeeded, {
          allowedTags: [],
          allowedAttributes: {},
        }),
      };
    ...
    try {
      ...
      if (course) {
        if (course.userId === user.id) {
          ...
          res
            .status(403)
            .json({ error: "You are not authorised to update this course." });
        }
      } else {
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (course) {
      if (course.userId === user.id) {
        ...
        res
          .status(403)
          .json({ error: "You are not authorised to delete this course." });
      }
    } else {
      const err = new Error(`Course Not Found`);
      res.status(404).json({ error: err.message });
